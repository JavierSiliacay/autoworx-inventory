...
6. **Smart Movement Types & Unit Cost Adjustments (\`Adj (Cost)\`)**:
   ...
   - **\`Adjustment (Cost)\`** (\`Adj (Cost)\`): **Unit Cost (Price) Adjustment only**. Used when correcting an item's unit cost (e.g. when an item like *Red Oxide Primer* is deducted via \`Adjustment (-)\` and transferred to/used in a mixing product). Selecting **Adj (Cost)** directly updates the unit cost of the mixing product in Master Inventory without needing to navigate to the Master Inventory page manually. It leaves physical stock quantity unchanged and is **NOT included in the Total Purchase amount calculation**.
