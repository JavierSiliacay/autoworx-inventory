...
6. **Smart Movement Types & Unit Cost Adjustments (\`Adj (Cost)\`)**:
   ...
   - **\`Adjustment (Cost)\`** (\`Adj (Cost)\`): **Unit Cost (Price) Adjustment only**. Used when an item is deducted and its cost is transferred to a mixing item. Example: If *Red Color Paint* is deducted (-1 quantity with cost ₱400), and *METALLIC RED GRAY MIXING* needs to inherit or set that unit cost to ₱400, selecting **Adj (Cost)** allows the staff to edit the unit cost directly to ₱400 for the mixing item. This updates the Master Inventory unit cost directly without needing to navigate to Master Inventory manually. It keeps the line-item total non-editable and is **NOT included in the Total Purchase calculation**.
